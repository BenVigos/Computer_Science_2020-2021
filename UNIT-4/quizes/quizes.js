...
function letter_frequency(text, letter){
    freq = 0;

    for (let i=0; i<text.length; i++){
        if (text[i] == letter){freq++}
    }
    return freq
}
...
function degree_clock(inp_angle) {
    let hr = 0;
    let min = 0;
    let angle = 0;
    ...
    for (let i = 0; i <= 360; i++) {
        ...
        if ((m_deg - h_deg) >= 360) {
            temp = Math.floor((m_deg - h_deg) / 360);
            angle = (m_deg - h_deg) - 360 * temp;
        } else {
            angle = m_deg - h_deg
        }

        // console.log(inp_angle, angle);
        // console.log("angle", i, ":", angle);
        // console.log("hour is: ", Math.floor(i/60), " and minutes are: ", i-(60*Math.floor(i/60)));
        // console.log("");

        // HOURHAND.style.transform = "rotate(" + h_deg + "deg)";
        // MINUTEHAND.style.transform = "rotate(" + m_deg + "deg)";
        if (inp_angle == angle) {
            out = Math.floor(i / 60).toString() + ":" + (i - (60 * Math.floor(i / 60))).toString()
            return out
        }
